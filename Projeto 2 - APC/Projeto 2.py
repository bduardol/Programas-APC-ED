...
def diciplinas(n):
 ...
 #CIC212,MATEMATICA,A,3
    ...
    #print(valor_repetidos)
    dados_final_print=[]
    
    for i in valor_repetidos:
        b=i.split(',')
        if int(b[3]) >= int(n):
            dados_final_print.append(b)  
    dados_final_print = sorted(dados_final_print, key= lambda t: t[2])
    dados_final_print = sorted(dados_final_print, key= lambda t: t[0])
    #print(dados_final_print)  
    aux=''
    aux_in=1
    ...
    for i in dados_final_print:
        ...
        if int(i[3]) >= int(n):
            kkk=i[0]+','+i[2]
            if i == dados_final_print[len(dados_final_print)-1]: #remover end=''do final da frase
                if i[0]+','+i[2] != aux and aux_in and i[0] != 'Nome': #novo
                    print(f' * {i[0]} ({i[2]}): {i[1]} ({i[3]})')
                    aux_in = 0
                    aux = i[0]+','+i[2]
                elif i[0]+','+i[2] != aux and i[0] != 'Nome': #novo
                    print(f'\n * {i[0]} ({i[2]}): {i[1]} ({i[3]})')
                    aux = i[0]+','+i[2]
                elif i[0]+','+i[2]==aux and i[0] != 'Nome': #novo
                    print(f', {i[1]} ({i[3]})')
...
def matricula(n):
    ...
    lista_um = sorted(lista, key= lambda t: t[1])
    teste12=[]
    teste12 = sorted(lista_um, key= lambda t: t[0], reverse=True)
